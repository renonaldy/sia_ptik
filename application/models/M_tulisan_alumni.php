...
class M_tulisan_alumni extends CI_Model{
	...
	function simpan_tulisan_alumni($judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("insert into tbl_alumni(alumni_judul,alumni_isi,alumni_kategori_id,alumni_kategori_nama,alumni_pengguna_id,alumni_author,alumni_gambar,alumni_slug,alumni_status) values ('$judul','$isi','$kategori_id','$kategori_nama','$user_id','$user_nama','$gambar','$slug','1')");
		return $hsl;
	}
	function get_tulisan_alumni_by_kode($kode){
		$hsl=$this->db->query("SELECT tbl_alumni.*,DATE_FORMAT(alumni_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_alumni where alumni_id='$kode'");
		return $hsl;
	}
	function update_tulisan_alumni($alumni_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("update tbl_alumni set alumni_judul='$judul',alumni_isi='$isi',alumni_kategori_id='$kategori_id',alumni_kategori_nama='$kategori_nama',alumni_pengguna_id='$user_id',alumni_author='$user_nama',alumni_gambar='$gambar',alumni_slug='$slug',alumni_status='1' where alumni_id='$alumni_id'");
		return $hsl;
	}
	function update_tulisan_alumni_tanpa_img($alumni_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$slug){
		$hsl=$this->db->query("update tbl_alumni set alumni_judul='$judul',alumni_isi='$isi',alumni_kategori_id='$kategori_id',alumni_kategori_nama='$kategori_nama',alumni_pengguna_id='$user_id',alumni_author='$user_nama',alumni_slug='$slug',alumni_status='1' where alumni_id='$alumni_id'");
		return $hsl;
	}
	function hapus_tulisan_alumni($kode){
	...
	function get_tulisan_by_kategori($kategori_id){
		$hsl=$this->db->query("SELECT tbl_alumni.*,DATE_FORMAT(alumni_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_alumni where alumni_kategori_id='$kategori_id'");
		return $hsl;
	}

	function get_tulisan_by_kategori_perpage($kategori_id,$offset,$limit){
		$hsl=$this->db->query("SELECT tbl_alumni.*,DATE_FORMAT(alumni_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_alumni where alumni_kategori_id='$kategori_id' limit $offset,$limit");
		return $hsl;
	}
    ...
    function count_good($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_alumni SET alumni_rating=alumni_rating+1 where alumni_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_like($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_alumni SET alumni_rating=alumni_rating+2 where alumni_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_love($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_alumni SET alumni_rating=alumni_rating+3 where alumni_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_genius($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_alumni SET alumni_rating=alumni_rating+4 where alumni_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function get_tulisan_populer(){
		$hasil=$this->db->query("SELECT tbl_alumni.*,DATE_FORMAT(alumni_tanggal,'%d %M %Y') AS tanggal FROM tbl_alumni ORDER BY alumni_views DESC limit 10");
		return $hasil;
	}
	...
	function get_kategori_for_blog(){
		$hasil=$this->db->query("SELECT COUNT(alumni_kategori_id) AS jml,kategori_id,kategori_nama FROM tbl_alumni JOIN tbl_kategori ON alumni_kategori_id=kategori_id GROUP BY alumni_kategori_id");
		return $hasil;
	}
