<?php
class M_tulisan_dosen extends CI_Model{

	function get_all_tulisan_dosen(){
		$hsl=$this->db->query("SELECT tbl_dosen.*,DATE_FORMAT(dosen_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_dosen ORDER BY id_dosen DESC");
		return $hsl;
	}
	function get_all_tulisan_dosen_kirim(){
		$hsl=$this->db->query("SELECT tbl_dosen.*,DATE_FORMAT(dosen_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_dosen ORDER BY id_dosen DESC");
		return $hsl;
	}

	function simpan_tulisan_dosen($nama_dosen,$nidn,$nik,$jenis_kelamin,$jabatan,$pangkat,$no_sertifikasi,$photo){
		$hsl=$this->db->query("insert into tbl_dosen(nama_dosen,nidn,nik,jenis_kelamin,jabatan,pangkat,no_sertifikasi,photo) values ('$nama_dosen','$nidn','$nik','$jenis_kelamin','$jabatan',''$pangkat,'$no_sertifikasi','$photo','1')");
		return $hsl;
	}
	function get_tulisan_dosen_by_kode($kode){
		$hsl=$this->db->query("SELECT tbl_dosen.*,DATE_FORMAT(dosen_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_dosen where id_dosen='$kode'");
		return $hsl;
	}
	function update_tulisan_dosen($akademik_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("update tbl_akademik set akademik_judul='$judul',akademik_isi='$isi',akademik_kategori_id='$kategori_id',akademik_kategori_nama='$kategori_nama',akademik_pengguna_id='$user_id',akademik_author='$user_nama',akademik_gambar='$gambar',akademik_slug='$slug',akademik_status='1' where akademik_id='$akademik_id'");
		return $hsl;
	}
	function update_tulisan_akademik_tanpa_img($akademik_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$slug){
		$hsl=$this->db->query("update tbl_akademik set akademik_judul='$judul',akademik_isi='$isi',akademik_kategori_id='$kategori_id',akademik_kategori_nama='$kategori_nama',akademik_pengguna_id='$user_id',akademik_author='$user_nama',akademik_slug='$slug',akademik_status='1' where akademik_id='$akademik_id'");
		return $hsl;
	}
	function hapus_tulisan_akademik($kode){
	...
	function get_tulisan_by_kategori($kategori_id){
		$hsl=$this->db->query("SELECT tbl_akademik.*,DATE_FORMAT(akademik_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_akademik where akademik_kategori_id='$kategori_id'");
		return $hsl;
	}

	function get_tulisan_by_kategori_perpage($kategori_id,$offset,$limit){
		$hsl=$this->db->query("SELECT tbl_akademik.*,DATE_FORMAT(akademik_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_akademik where akademik_kategori_id='$kategori_id' limit $offset,$limit");
		return $hsl;
	}
    ...
    function count_good($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_akademik SET akademik_rating=akademik_rating+1 where akademik_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_like($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_akademik SET akademik_rating=akademik_rating+2 where akademik_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_love($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_akademik SET akademik_rating=akademik_rating+3 where akademik_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_genius($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_akademik SET akademik_rating=akademik_rating+4 where akademik_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function get_tulisan_populer(){
		$hasil=$this->db->query("SELECT tbl_akademik.*,DATE_FORMAT(akademik_tanggal,'%d %M %Y') AS tanggal FROM tbl_akademik ORDER BY akademik_views DESC limit 10");
		return $hasil;
	}
	...
	function get_kategori_for_blog(){
		$hasil=$this->db->query("SELECT COUNT(akademik_kategori_id) AS jml,kategori_id,kategori_nama FROM tbl_akademik JOIN tbl_kategori ON akademik_kategori_id=kategori_id GROUP BY akademik_kategori_id");
		return $hasil;
	}
