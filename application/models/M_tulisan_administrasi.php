<?php
class M_tulisan_administrasi extends CI_Model{

	function get_all_tulisan_administrasi(){
	...
	function get_all_tulisan_administrasi_kirim(){
	...
	function simpan_tulisan_administrasi($judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("insert into tbl_administrasi(administrasi_judul,administrasi_isi,administrasi_kategori_id,administrasi_kategori_nama,administrasi_pengguna_id,administrasi_author,administrasi_gambar,administrasi_slug,administrasi_status) values ('$judul','$isi','$kategori_id','$kategori_nama','$user_id','$user_nama','$gambar','$slug','1')");
		return $hsl;
	}
	function get_tulisan_administrasi_by_kode($kode){
	...
	function update_tulisan_administrasi($administrasi_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		$hsl=$this->db->query("update tbl_administrasi set administrasi_judul='$judul',administrasi_isi='$isi',administrasi_kategori_id='$kategori_id',administrasi_kategori_nama='$kategori_nama',administrasi_pengguna_id='$user_id',administrasi_author='$user_nama',administrasi_gambar='$gambar',administrasi_slug='$slug',administrasi_status='1' where administrasi_id='$administrasi_id'");
		return $hsl;
	}
	function update_tulisan_administrasi_tanpa_img($administrasi_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$slug){
		$hsl=$this->db->query("update tbl_administrasi set administrasi_judul='$judul',administrasi_isi='$isi',administrasi_kategori_id='$kategori_id',administrasi_kategori_nama='$kategori_nama',administrasi_pengguna_id='$user_id',administrasi_author='$user_nama',administrasi_slug='$slug',administrasi_status='1' where administrasi_id='$administrasi_id'");
		return $hsl;
	}
	function hapus_tulisan_administrasi($kode){
	...
	function get_tulisan_by_kategori($kategori_id){
		$hsl=$this->db->query("SELECT tbl_administrasi.*,DATE_FORMAT(administrasi_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_administrasi where administrasi_kategori_id='$kategori_id'");
		return $hsl;
	}

	function get_tulisan_by_kategori_perpage($kategori_id,$offset,$limit){
		$hsl=$this->db->query("SELECT tbl_administrasi.*,DATE_FORMAT(administrasi_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_administrasi where administrasi_kategori_id='$kategori_id' limit $offset,$limit");
		return $hsl;
	}
    ...
    function count_good($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_administrasi SET administrasi_rating=administrasi_rating+1 where administrasi_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_like($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_administrasi SET administrasi_rating=administrasi_rating+2 where administrasi_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_love($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_administrasi SET administrasi_rating=administrasi_rating+3 where administrasi_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function count_genius($kode){
        ...
        if($cek_ip->num_rows() <= 0){
            $this->db->trans_start();
				...
				$this->db->query("UPDATE tbl_administrasi SET administrasi_rating=administrasi_rating+4 where administrasi_id='$kode'");
			$this->db->trans_complete();
			if($this->db->trans_status()==TRUE){
				return TRUE;
			}else{
    ...
    function get_tulisan_populer(){
		$hasil=$this->db->query("SELECT tbl_administrasi.*,DATE_FORMAT(administrasi_tanggal,'%d %M %Y') AS tanggal FROM tbl_administrasi ORDER BY administrasi_views DESC limit 10");
		return $hasil;
	}
	...
	function get_kategori_for_blog(){
		$hasil=$this->db->query("SELECT COUNT(administrasi_kategori_id) AS jml,kategori_id,kategori_nama FROM tbl_administrasi JOIN tbl_kategori ON administrasi_kategori_id=kategori_id GROUP BY administrasi_kategori_id");
		return $hasil;
	}
